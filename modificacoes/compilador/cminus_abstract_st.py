...
class Simple_exp(AbstractStNode):
    def __init__(self, exp_left=None, relational=None, exp_right=None,
                 generic_exp=None, line=-1):
        super().__init__(line)
...
class AbstractStNodeConstructVisitor(cminusVisitor):
    """
    Converte a árvore gerada pelo ANTLR em uma AST própria do compilador.
    Os nomes dos métodos visitXXX devem permanecer iguais aos nomes gerados
    pelo ANTLR a partir da gramática cminus.g4.
    """
    ...
    def visitParams(self, ctx: cminusParser.ParamsContext):
        return Params(
            par_list=[self.visit(param) for param in ctx.params_list]
            if not ctx.VOID()
            else [],
            line=ctx.start.line,
        )
    ...
    def visitComp_decl(self, ctx: cminusParser.Comp_declContext):
        return Comp_decl(
            local_decl=[self.visit(decl) for decl in ctx.loc_decls]
            if ctx.local_decl()
            else [],
            stmt_list=[self.visit(stmt) for stmt in ctx.stmts_list]
            if ctx.stmt_list()
            else [],
            line=ctx.start.line,
        )
    ...
    def visitStmt(self, ctx: cminusParser.StmtContext):
        if ctx.exp_decl():
            tipo_comando = self.visit(ctx.exp_decl())
        elif ctx.comp_decl():
            tipo_comando = self.visit(ctx.comp_decl())
        elif ctx.select_decl():
            tipo_comando = self.visit(ctx.select_decl())
        elif ctx.iter_decl():
            tipo_comando = self.visit(ctx.iter_decl())
        elif ctx.ret_decl():
            tipo_comando = self.visit(ctx.ret_decl())
        else:
            tipo_comando = None

        return Stmt(
            stmt_type=tipo_comando,
            line=ctx.start.line,
        )
    ...
    def visitSelect_decl(self, ctx: cminusParser.Select_declContext):
        return Select_decl(
            ...
            if_body=[self.visit(stmt) for stmt in ctx.if_body],
            else_body=[self.visit(stmt) for stmt in ctx.else_body]
            if ctx.else_body
            else [],
            line=ctx.start.line,
        )
    ...
    def visitRelational(self, ctx: cminusParser.RelationalContext):
        ...
        if ctx.LET():
            operador = ctx.LET().getText()
        elif ctx.LT():
            operador = ctx.LT().getText()
        elif ctx.GT():
            operador = ctx.GT().getText()
        elif ctx.GET():
            operador = ctx.GET().getText()
        elif ctx.EQ():
            operador = ctx.EQ().getText()
        elif ctx.NEQ():
            operador = ctx.NEQ().getText()
